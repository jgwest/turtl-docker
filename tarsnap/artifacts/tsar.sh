...
export SCRIPT_ROOT=`cd $SCRIPT_ROOT;pwd`

#export KEY_FILE=/tarsnap/tarsnap.key
#export CACHE_DIR=/tarsnap/cache

if [ -z "$ARCHIVE_SUFFIX" ]; then
	echo "Error: Archive suffix environment variable was not set."
	exit
fi
...
# Licensed under GPL-v3.0, see above repository for license details.
#
...
TARSNAP_R="tarsnap -v --list-archives --cachedir $CACHE_DIR --keyfile $KEY_FILE | grep $ARCHIVE_SUFFIX " # for getting archive list
TARSNAP_D="tarsnap -d --humanize-numbers --cachedir $CACHE_DIR --keyfile $KEY_FILE" # for deleting, archive names will be appended here
...
if [ $? -gt 0 ]; then
    echo Caught error \(listed above\), exiting.
    shred -u "$LIST"
    exit 3
fi
